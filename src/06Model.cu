						 ...
						 int* type_buffer, int cellNumber)
{
	...
	const double Ko=5.4;////luo
	double Gks, Gto;////luo

	double IKr=0,IKs=0,IK1=0,Ito=0,INa=0,IbNa=0,ICaL=0,IbCa=0,INaCa=0,IpCa=0,IpK=0,INaK=0,Irel=0,Ileak=0,Iup=0,Ixfer=0;////luo
	...
	if(type== ENDO) Gto=0.073;////luo
	if(type== MCELL || type== EPI)  Gto=0.294;////luo
	Ito=Gto*sr*ss*(V-Ek);////luo
    
	IKr=Gkr*sqrt(Ko/5.4)*sxr1*sxr2*(V-Ek);////luo

	
	if(type== ENDO ||type== EPI)  Gks=0.392; ////luo
	if(type== MCELL)  Gks=0.098; ////luo
	...
	{
		//out_itot=IKr+IKs+IK1+Ito+INa+IbNa+ICaL+IbCa+INaK+INaCa+IpCa+IpK+Istim;	////luo
		out_itot=IKr+IKs+IK1+Ito+INa+IbNa+ICaL+IbCa+INaK+INaCa+IpCa+IpK+Istim+INa_l;	////luo
	}
	...
	dNai=-(INa+IbNa+3*INaK+3*INaCa)*inverseVcF*CAPACITANCE;////luo
	Nai+=dt*dNai;////luo
    
	dKi=-(Istim+IK1+Ito+IKr+IKs-2*INaK+IpK)*inverseVcF*CAPACITANCE;////luo
	Ki+=dt*dKi;////luo
