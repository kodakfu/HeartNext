...
void Lua::object(const LuaClass *object, const std::string& name) {
	// prevent from stack is not empty when return value from function
	lua_getglobal(vm, "_G");
    load(name);
    table();
    load("mtab", -2);
...
void Lua::string(const std::string& string) {
    lua_pushlstring(vm, string.c_str(), string.size());
}
...
std::string Lua::string(const int i) {
	...
	size_t len = 0;
	const char* iter = lua_tolstring(vm, i, &len);
    return std::string(iter, iter+len);
}
