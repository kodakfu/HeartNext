...
protected:
    ...
    template <class T>
    T arg(const int i, std::true_type) {
		 typedef typename std::remove_reference<T>::type Tbase;
        if (std::is_base_of<util::LuaClass, Tbase>::value) {
            return *(Tbase *)object(i);
        } else {
            return *(Tbase *)userdata(i);
        }
    }
	template <class T>
    T arg(const int i, std::false_type) {
    ...
    std::tuple<T, T1, Args...> args(const int i = 1) {
        T t = arg<T>( i, typename std::is_reference<T>::type() );
        return std::tuple_cat( std::forward_as_tuple(t), args<T1, Args...>( i + 1));
    }
    ...
    template <int N> struct apply_function {
        ...
        static R apply(R (*function)(FunctionArgs...),
            std::tuple<TupleArgs...>& t, Args&&... args) {
			typedef typename std::tuple_element<N-1, std::tuple<TupleArgs...> >::type nextType;
			nextType theParam = std::get<N-1>(t);
            return
                apply_function<N-1>::apply(function, t, theParam, args...);
        }
    };
...
template <>
std::string Lua::arg<std::string>(const int i, std::false_type);

template <>
lua_Number Lua::arg<lua_Number>(const int i, std::false_type);

template <>
lua_Integer Lua::arg<lua_Integer>(const int i, std::false_type);
template <>
int Lua::arg<int>(const int i, std::false_type);

template <>
bool Lua::arg<bool>(const int i, std::false_type);
