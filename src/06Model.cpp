...
static int return06ParamSpec(lua_State* s){
	lua_newtable(s);
	Helper::map2table(s, ParamSpec);
	return 1;
}
static int UpdateVentricleCurrent_06Model(lua_State* s)
{
...
extern "C" int load06Model(lua_State* s){
	static util::Lua vm(s);
	lua_pushcfunction(s, return06ParamSpec);
	lua_setglobal(s, "return06ParamSpec");
	lua_pushcfunction(s, UpdateVentricleCurrent_06Model);
	lua_setglobal(s, "UpdateVentricleCurrent_06Model");
